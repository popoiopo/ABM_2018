...
to turtle-setup ;; turtle procedure
  ...
  set metabolism random-in-range 1 metabolism-max
  set vision random-in-range 1 vision-max
...
to go
  ...
  let missing-turtles initial-population - (count turtles)
  print missing-turtles
  if missing-turtles > 0
  [
    create-turtles missing-turtles [ turtle-setup ]
  ]
  tick
end
...
to patch-growback ;; patch procedure
  ...
  ifelse Model = "ImmediateGrowback"
  [
  set psugar max-psugar
  ]
  [
...
Model
Model
"ImmediateGrowback" "ConstantGrowback"
0
